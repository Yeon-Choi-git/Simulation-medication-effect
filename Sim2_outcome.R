...
######## My directory ########
setwd("H:/NEO/Results")
neo.original<-read.dta("H:/NEO/NEO_analyse_PP157.dta")

##### Saskia's directory #####
#setwd("D:/epidemiologie/Jan Choi/selectie/NEO analysis")
#neo.original<-read.dta("NEO_analyse_PP157.dta")
...
###                                                ###
######################################################
### new variable glucose medication use
# Brrowing variables
neo.original$medgluc <- as.factor(ifelse(neo.original$medA10gluclow !="no", "yes", "no") )
table(neo.original$medgluc)

### Select only the complete cases for those variables which are interesting for us
### Exclude female-only measurement
### Exclude IMT measurement 
neo.pre<-na.omit(neo.original[,c("weegfactor1"    
                             ,"sexe"           
                             ,"leeftijd"       
                             ,"lengtem"        
                             ,"gewichtm"       
